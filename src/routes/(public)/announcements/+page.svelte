...
<section class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:py-20">
	<SectionHeader
		eyebrow={m.announcements_page_eyebrow()}
		title={m.announcements_page_title()}
		description={m.announcements_page_description()}
	/>
	...
	{:then announcements}
		{#if announcements.length === 0}
			<div class="rounded-md border border-dashed border-border bg-card px-6 py-16 text-center">
				...
				<p class="display mt-5 text-xl">{m.announcements_page_empty_title()}</p>
				<p class="mx-auto mt-3 max-w-md text-sm leading-relaxed text-muted-foreground">
					{m.announcements_page_empty_body()}
					<a href={site.phoneHref} class="font-mono text-foreground hover:underline" dir="ltr"
						>{site.phone}</a
					>
				</p>
				<div class="mt-6">
					<Button href="/services" class="btn-label">{m.announcements_page_cta()}</Button>
				</div>
			</div>
		{:else}
			<div class="grid gap-5">
