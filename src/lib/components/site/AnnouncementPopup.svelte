<script lang="ts">
	...
	const plate = $derived(announcement ? datePlate(announcement.starts_at) : null);
	const href = $derived(
		announcement ? (announcement.link_url ?? `/announcements#${announcement.id}`) : '#'
	);
	const label = $derived(
		announcement?.link_label ?? (announcement?.link_url ? 'Ayrıntılar' : 'Duyuruya git')
	);
...
<div class="sr-only" aria-live="polite">
	{#if announcement && open}Yeni duyuru: {announcement.title}{/if}
</div>
